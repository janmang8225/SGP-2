...
class SgpSem4SupabaseUser extends BaseAuthUser {
  SgpSem4SupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> sgpSem4SupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SgpSem4SupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
